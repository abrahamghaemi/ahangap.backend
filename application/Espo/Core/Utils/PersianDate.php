...
class PersianDate
{
     ...
     *
     */
    public static function toGregorian($dateTime)
    {
        ...
        if(strlen($dateTime) > 10){
            ...
            $dateWithFormat = explode("-", $date[0]);
        } else {
            $dateWithFormat = explode("-", $dateTime);
        }

        $j_y = $dateWithFormat[0];
        $j_m = $dateWithFormat[1];
        $j_d = $dateWithFormat[2];
        ...
        $convertDate = implode('-', [$gy, $gm, $gd]);


        if(strlen($dateTime) > 10){
            return $convertDate . " " . $date[1];
        }
...
//echo PersianDate::toGregorian("1396-01-13");
//echo PersianDate::toGregorian("1396-01-13 00:58:42");
//file_put_contents('/home/vagrant/Code/patron/data/logs/logs.log',print_r([$dateStart, $dateEnd, $dateEndDate, $dateStartDate, $entity->getValues()],true), FILE_APPEND);
